...
int Playing(vector<string> dict, int PlayTime, int DictSize)
{
    ...
    while (time(NULL) - StartTime < PlayTime
           and ExitFlag == 0) { //пока разность текущего времени и времени
                                //старта игры
        ...
        string InputWord;
        bool Equal = false; //флаг для того, чтобы в случае неверного ввода
                            //слова, игрок вводил то же самое слово, а не
                            //выбиралось случайное
        while (!Equal and (time(NULL) - StartTime < PlayTime)
               and ExitFlag == 0) { //пока игрок не ввёл правильное слово и пока
                                    //время не завершится и пока игрок не
                                    //завершит игру
            cin >> InputWord; //ввод слова игроком
            if (InputWord == ":q") {
                wrong--;
                ExitFlag = 1;
            } //для досрочного завершения игры (в случае, если игрок введёт
              //слово
              //":q", игра досрочно завершится
            if (InputWord == NewWord) {
                score++;
                Equal = true; //если слово введено верно, игрок получает +1 к
                              //числу правильных слов и выбирается новое слово
            } else {
                system("clear");
                wrong++;
                InfoOutput(
                        ...
                        true);
                cout << "\n\n\n\t" << NewWord << "\n\n\n\n";
            } //если слово введено неверно, флаг Equal не меняется,
              //следовательно игрок должен ввести то же самое слово
        }
        system("clear");
    }
